document.addEventListener('DOMContentLoaded', function() {
    ...
    if (menuToggleButton && mobileMenuContent) {
        menuToggleButton.addEventListener('click', function() {
            ...
            // NOVO: Atualiza o estado ARIA após o toggle
            enhanceA11y(); 
        });
        ...
        navMobileLinks.forEach(link => {
            link.addEventListener('click', () => {
                ...
                // NOVO: Atualiza o estado ARIA após o fechamento
                enhanceA11y(); 
            });
        });
    }

// ... o restante das funções de template ...
    ...
    function loadPage() {
        ...
        // **NOVO**: Garante foco acessível na mudança de página
        enhanceA11y(); 
    ...
    // **NOVO**: Configura o botão de alternância de tema ao carregar
    setupThemeToggle();
...
/* ======================================================= */
/* E. ACESSIBILIDADE E TEMAS (WCAG 2.1)                    */
/* ======================================================= */

/**
 * Adiciona um botão de alternância de tema e gerencia o estado no localStorage.
 */
function setupThemeToggle() {
    ...
    // Utiliza o estilo do botão primário, mas garantindo que o texto seja legível
    themeButton.className = 'btn btn-primary'; 
    themeButton.setAttribute('aria-label', 'Alternar tema: Claro, Escuro, Alto Contraste');
    themeButton.innerHTML = '🌓 Tema'; 
    
    // Adiciona o botão ao header (antes do botão hambúrguer)
    const menuButton = document.getElementById('menu-toggle');
    headerMain.insertBefore(themeButton, menuButton);

    // 2. Define o tema inicial (usa <html> para aplicar o data-theme)
...
/**
 * Atualiza atributos ARIA e gerencia o foco ao carregar novo conteúdo na SPA (WCAG 2.4.3).
 */
function enhanceA11y() {
    // 1. Gerenciamento de Foco para SPA (Move o foco para o conteúdo principal após a troca de página)
    const mainContent = document.getElementById('app-content');
    if (mainContent) {
        // Torna o elemento focalizável temporariamente e move o foco
        mainContent.setAttribute('tabindex', '-1'); 
        mainContent.focus(); 
        mainContent.removeAttribute('tabindex'); // Remove o tabindex após o foco
    }

    // 2. Atualiza ARIA para Navegação do Menu Hambúrguer
    const menuToggleButton = document.getElementById('menu-toggle'); 
    const mobileMenuContent = document.getElementById('mobile-menu-content'); 
    
    if (menuToggleButton && mobileMenuContent) {
        const isOpen = mobileMenuContent.classList.contains('active');
        menuToggleButton.setAttribute('aria-expanded', isOpen ? 'true' : 'false');
        mobileMenuContent.setAttribute('aria-hidden', isOpen ? 'false' : 'true');
    }
}